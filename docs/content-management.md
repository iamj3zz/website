...
- `hero_image` - Set to `true` to display a hero image on the work's print layout (only when the hero.jpg file exists at `assets/works/YYYY-MM-DD-slug/hero.jpg`). If not set or false, no hero image is rendered.
