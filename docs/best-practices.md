...
commissioned: false  # or true - indicates if work had specific requirements (workshops always false)
...
5. **Place hero image (print only, optional)**: If the work has a print hero image, place `hero.jpg` in `assets/works/YYYY-MM-DD-slug/` and set `hero_image: true` in front matter
6. **Test locally**: Run `bundle exec jekyll serve` and verify display
7. **Check printable version**: View works.markdown to ensure proper print display
...
- **Work 31** (`_portfolio/1900-01-01-complete-template.md`): **⭐ ULTIMATE TEMPLATE** - Comprehensive reference demonstrating:
  ...
  - Multi-category work setup (installations, films, live-acts, performances, residencies, releases, workshops)
