...
- `robots.txt` - Search engine crawler configuration with sitemap declaration; also disallows known AI-training crawlers (GPTBot, CCBot, ClaudeBot, Google-Extended, etc.) — see "AI-Scraping & Rights Protection Workflow" in `CLAUDE.md`
...
- `_pages/licensing.markdown` - Licensing & rights / AI-training opt-out notice at `/licensing/` (noindex, footer-linked, same treatment as privacy)
...
1. **`_includes/seo.html`** - Main SEO include with:
   ...
   - **Robots meta tag** (conditional on `page.noindex` front matter):
     - `noindex, nofollow` for pages with `noindex: true` (404, privacy, licensing)
...
2. **Structured Data (Schema.org):**
   - **Portfolio works** get `CreativeWork` structured data (via `seo.type: CreativeWork` in `_config.yml` defaults)
     ...
     - Includes: name, description, image, URL, author (with `sameAs` social links), license, usageInfo, copyrightNotice, creditText, datePublished, dateModified, genre
     ...
     - `license`/`usageInfo` point to `site.license_url` (`/licensing/`); `copyrightNotice` from `site.copyright_notice` — both configured in `_config.yml`
   ...
   - **Visual artworks** get `VisualArtwork` structured data (via `seo.type: VisualArtwork` in `_config.yml` defaults)
     ...
     - Includes: name, description, image, URL, creator (with `sameAs` social links), license, usageInfo, copyrightNotice, creditText, medium, dateCreated
...
5. **Robots & Indexing:**
   - **robots.txt:**
     ...
     - Allows all crawlers for search indexing: `User-agent: * / Allow: /`
     - Disallows ~20 known AI-training crawlers by name (`GPTBot`, `ChatGPT-User`, `CCBot`, `ClaudeBot`, `Claude-Web`, `anthropic-ai`, `Google-Extended`, `Applebot-Extended`, `Bytespider`, `Amazonbot`, `Meta-ExternalAgent`, `Meta-ExternalFetcher`, `PerplexityBot`, `Diffbot`, `ImagesiftBot`, `Timpibot`, `cohere-ai`, `Omgilibot`, `Omgili`, `YouBot`) — only the AI-training-specific variants (`-Extended` etc.), never bare `Applebot`/`Googlebot`, so normal search indexing is untouched
     - Declares sitemap location: `Sitemap: https://www.j3zz.com/sitemap.xml`
   - **Sitemap with Bilingual Alternates:**
     - Generated by `generate-sitemap.rb` Ruby script (runs after Jekyll build)
     - Includes all pages, portfolio works, and artwork items
     - For bilingual pages: includes `<xhtml:link rel="alternate" hreflang="...">` tags declaring language alternates
     - Enables Google to crawl and index both EN/FR versions as related pages
     - Format: ISO 8601 XML with xhtml namespace
     - Respects `published: false` and `sitemap: false` flags (items are excluded) — note: does NOT check `noindex`, so noindexed pages like `/privacy/` and `/licensing/` still appear in the sitemap
   - **Page-level noindex:**
     - Pages with `noindex: true` in front matter emit `<meta name="robots" content="noindex, nofollow">`
     - Currently applied to: `404.html`, `_pages/privacy.markdown`, and `_pages/licensing.markdown` (+ FR equivalents)
     - Other pages default to `index, follow`
   - **AI/TDM opt-out:** see "AI-Scraping & Rights Protection Workflow" in `CLAUDE.md` for the full picture (robots.txt rules above + `noai`/`noimageai` meta tag + JSON-LD rights fields + `/licensing/` page + embedded image metadata)
...
copyright_notice: "© 2000–2026 J3ZZ. All Rights Reserved."
...
| *(none)* | *(universal only)* | `/privacy/`, `/licensing/` |
