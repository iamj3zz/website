...
## Commissioned Field (Required for All Works)

All portfolio works must include the `commissioned` field in the front matter (outside the metadata section):
...
**Commissioned Status:**
- `commissioned: true` - Work created with **specific requirements/instructions** from a client, commissioner, or curator
- `commissioned: false` - Work created with **full creative freedom** (self-initiated, collaborative partnerships, artistic freedom)

**Key Distinction:**
- **TRUE** = Specific requirements, brief, instructions from client/commissioner
- **FALSE** = Full creative freedom (even if paid, invited, or supported financially)

**Examples:**
- Museum installation with specific brief → `commissioned: true`
- Self-initiated art project → `commissioned: false`
- Band membership (collaborative partnership) → `commissioned: false`
- Film score with director's requirements → `commissioned: true`
- Residency program (always creative freedom) → `commissioned: false`
