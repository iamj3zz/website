...
- `scripts/generate-webp.sh` (WebP performance pipeline, see [Code Quality](docs/code-quality.md)) also embeds this same rights metadata into every `.webp` it generates, via the same `embed-image-rights.sh`. It's wired into the identical Lefthook pattern: a `generate-webp` pre-commit command (`lefthook.yml`) runs on any staged `assets/**/*.{jpg,jpeg,png}` with `--stage`, so new images automatically get a rights-protected `.webp` sibling generated and committed alongside them. No-ops gracefully if `cwebp` isn't installed (`sudo apt-get install webp`). One-time backfill for pre-existing images: `./scripts/generate-webp.sh` once, locally.
