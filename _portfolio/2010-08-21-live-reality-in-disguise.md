---
published: true
status: major
layout: work
print_hero_image: true
title: REALITY IN DISGUISE
work_id: reality-in-disguise
abstract: 'One of seven flagship productions in the Art''n''Go initiative — a live
  interdisciplinary show co-created with emerging artists from 11 European countries,
  blending violin, composition, contemporary dance, spoken word, and visual arts.
  Developed through residencies in Germany and performed across major European festivals
  in 2010–2011.

  '
description: One of seven flagship productions in the Art'n'Go initiative — a live
  interdisciplinary show co-created with emerging artists from 11 European countries,
  blending violin, composition, contemporary dance, spoken word, and visual arts.
  Developed through residencies in Germany and performed across major European festivals
  in 2010–2011.
category: live-acts
commissioned: false
image: "/assets/works/2010-08-21-live-reality-in-disguise/thumbnail.jpg"
metadata:
  date: 2009–2011
  based_in: Remscheid & Heek, Germany; Florence, Italy; Budapest, Hungary; Cologne,
    Germany
  role: violinist, composer, co-creator
  collaborators: 'ROOTS & ROUTES international artist network (11 European countries:
    France, Germany, Hungary, Romania, Italy, Netherlands, Spain, Greece, Belgium,
    UK, plus local collaborators in each residency location)'
  custom:
  - label: Programme
    value: Art'n'Go – New Routes to Intercultural Creativity
    link: https://www.rootsnroutes.eu/projects-id64
  - label: Funding
    value: EU CULTURE Programme (2007–2013)
    link: https://culture.ec.europa.eu/resources/creative-europe-previous-programmes/culture-programme-2007-2013
  - label: Project Period
    value: September 2009 – August 2011
  - label: Extended Residency
    value: Heek, Germany (artistic development space)
  - label: Performance Premiere
    value: Cologne III – One Night and a Million Sounds of Migration (Aug 21, 2010)
    link: http://rootsnroutes.tv/video-id1045-vpage=9
  - label: Performance
    value: Fabbrica Europa Festival, Florence (May 20–21, 2011)
    link: https://fabbricaeuropa.net/en/
  - label: Performance
    value: Sziget Festival (A38 stage), Budapest (Aug 9, 2011)
    link: https://szigetfestival.com/en/
  - label: 2011 Official News & Video
    value: ROOTS & ROUTES announcement with Sziget performance documentation
    link: https://www.rootsnroutes.eu/news-id52
  - label: Fabbrica Europa 2011 Event Page
    value: Reality in Disguise + Traces + About Traffic performance documentation
    link: https://fabbricaeuropa.net/en/events/reality-in-disguise-traces-about-traffic/
sections:
- type: hero-image
  image: "/assets/works/2010-08-21-live-reality-in-disguise/hero.jpg"
- type: iframe
  embed_code: <iframe width="560" height="315" src="https://www.youtube.com/embed/8oTRJNRk0tE"
    frameborder="0" allowfullscreen></iframe>
  responsive: true
  aspect_ratio: '16:9'
  caption: REALITY IN DISGUISE - Fabbrica Europa Festival - 2011
- type: description
- type: text
  content: |
    The production was directed by **Max Greyson** and **Ruben Vandendriessche**, with an ensemble of musicians, dancers, vocalists, and visual artists who had previously collaborated on earlier ROOTS & ROUTES initiatives. The show challenged the boundaries between musical genres and artistic disciplines, creating an immersive environment where classical violin met dub step, spoken word merged with visual projection, and live composition provided a fluid backbone for choreography and performance art.

    Between May and August 2011, the seven Art'n'Go productions were presented across major European festivals, positioning the project as a showcase of European cultural diversity and emerging artistic talent. The initiative represented a significant investment in intergenerational cultural exchange and the professionalization of young artists from economically and geographically diverse European contexts.
- type: iframe
  embed_code: <iframe width="560" height="315" src="https://www.youtube.com/embed/WkSSiRLR9FY"
    frameborder="0" allowfullscreen></iframe>
  responsive: true
  aspect_ratio: '16:9'
  caption: ART'N'GO mashup inlcuding REALITY IN DISGUISE - Sziget Festival (A38 stage)
    - 2011
- type: linked-events
- type: metadata
---
